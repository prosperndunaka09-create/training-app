...
import { LogOut, User, ChevronDown, Zap, LayoutDashboard, Wallet, ArrowDownToLine, UserCircle, Menu, X, Shield } from 'lucide-react';
const Navbar: React.FC = () => {
  ...
  return <nav className="sticky top-0 z-40 bg-[#0a0e1a]/90 backdrop-blur-xl border-b border-indigo-500/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {isAuthenticated && <div className="hidden md:flex items-center gap-1">
              {allNavItems.map(item => <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === item.id ? 'bg-indigo-500/15 text-indigo-400' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'}`}>
                  <item.icon size={16} />
                  {item.label}
                </button>)}
            </div>}
