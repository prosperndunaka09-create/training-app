...
import { DollarSign, Zap, Award, Wallet, ArrowDownToLine, TrendingUp, CheckCircle, Clock, Lock, ArrowRight, BarChart3, Target, GraduationCap, Star, RefreshCw, MessageCircle, AlertTriangle } from 'lucide-react';
import { DailyBonusCompact } from './DailyBonus';
import CombinationOrderModal from './CombinationOrderModal';

const Dashboard: React.FC = () => {
  const { user, tasks, wallets, withdrawals, loadTasks, loadWallets, loadWithdrawals, loadTransactions, setActiveTab, resetTrainingProgress, upgradeAccount, clearCombinationOrder } = useAppContext();
  ...
  useEffect(() => {
    ...
    loadWithdrawals();
    loadTransactions();
  }, [loadTasks, loadWallets, loadWithdrawals, loadTransactions]);
  ...
  const allTasksComplete = isTraining ? completedCount === 45 : completedCount === tasks.length;
  ...
  return (
    <div className="space-y-6">
          ...
          <p className="text-gray-400 text-sm">
            {isTraining
              ...
              : allTasksComplete
                ...
                : `You have ${tasks.length - completedCount} tasks remaining in your VIP${user?.vip_level || 1} set.`
            }
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-5 bg-white/[0.03] border border-white/[0.06] rounded-2xl hover:border-indigo-500/20 transition-all group cursor-pointer" onClick={() => setActiveTab('tasks')}>
          ...
          <p className="text-2xl font-bold text-white">{completedCount}<span className="text-sm text-gray-500 font-normal">/{isTraining ? 45 : tasks.length}</span></p>
        </div>

        <div className="p-5 bg-white/[0.03] border border-white/[0.06] rounded-2xl hover:border-indigo-500/20 transition-all group cursor-pointer" onClick={() => setActiveTab('profile')}>
          <div className="flex items-center justify-between mb-3">
        ...
        <div className="p-5 bg-white/[0.03] border border-white/[0.06] rounded-2xl hover:border-indigo-500/20 transition-all group cursor-pointer" onClick={() => setActiveTab('wallet')}>
          <div className="flex items-center justify-between mb-3">
      ...
      <div className="p-6 bg-white/[0.02] border border-white/[0.06] rounded-2xl">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-400">{completedCount} of {isTraining ? 45 : tasks.length} tasks completed</span>
        ...
        <div className="flex flex-wrap gap-1.5 mt-4">
          {(tasks.length > 0 ? tasks : Array.from({ length: isTraining ? 45 : 35 }, (_, i) => ({ task_number: i + 1, status: i === 0 ? 'pending' : 'locked' }))).map((task: any) => (
            <div
        ...
        {isTraining && !trainingComplete && (
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-white font-semibold">Reset Training</span>
            </div>
            <p className="text-xs text-gray-500">Reset all training progress</p>
          </button>
        )}
