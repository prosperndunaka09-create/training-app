...
import { ArrowDownToLine, AlertCircle, CheckCircle, Clock, XCircle, Loader2, DollarSign, Wallet, History, Lock } from 'lucide-react';

const WithdrawalSection: React.FC = () => {
  const { user, tasks, wallets, withdrawals, loadTasks, loadWallets, loadWithdrawals, requestWithdrawal, isLoading, setActiveTab } = useAppContext();
  ...
  useEffect(() => {
    ...
    loadWallets();
    loadWithdrawals();
  }, [loadTasks, loadWallets, loadWithdrawals]);

  const completedCount = tasks.filter(t => t.status === 'completed').length;
  const isTraining = user?.account_type === 'training';
  const isPersonal = user?.account_type === 'personal';
  
  // Training accounts: 45/45 tasks required
  // Personal accounts: 35/35 tasks required
  const trainingTasksComplete = completedCount === 45 && tasks.length === 45;
  const personalTasksComplete = completedCount === 35 && tasks.length === 35;
  
  const allTasksComplete = isTraining ? trainingTasksComplete : personalTasksComplete;
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    const success = await requestWithdrawal(numAmount, primaryWallet!.wallet_address);
    if (success) {
      setAmount('');
    }
  };
          ...
          <div className="flex flex-col gap-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
              ...
              {allTasksComplete ? (
                ...
                <><Lock size={14} className="text-amber-400" /><span className="text-amber-400 font-medium">{completedCount}/35 Tasks</span></>
              )}
            </div>
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  You need to complete all {isTraining ? 45 : 35} {isTraining ? 'training' : 'VIP1'} tasks before you can withdraw. You have completed {completedCount}/{isTraining ? 45 : 35} tasks.
                </p>
                <button
              ...
              <div key={w.id} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/[0.06] rounded-xl">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-600">{new Date(w.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </div>
